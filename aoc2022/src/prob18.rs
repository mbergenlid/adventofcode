...
pub fn solve_part_2(input: &str) -> usize {
    ...
    let x_minmax = cubes.iter().map(|(x, _, _)| x).minmax().into_option().unwrap();
    let y_minmax = cubes.iter().map(|(_, y, _)| y).minmax().into_option().unwrap();
    let z_minmax = cubes.iter().map(|(_, _, z)| z).minmax().into_option().unwrap();
    ...
    while let Some((x,y,z)) = queue.pop_front() {
        ...
        for n@(n_x, n_y, n_z) in [(x-1, y, z), (x+1, y, z), (x, y-1, z), (x, y+1, z), (x, y, z-1), (x,y, z+1)] {
            if n_x < x_bounds.0 || n_x > x_bounds.1 || n_y < y_bounds.0 || n_y > y_bounds.1 || n_z < z_bounds.0 || n_z > z_bounds.1 {
                continue;
            }
            if cubes.contains(&n) {
                sides_visible += 1;
